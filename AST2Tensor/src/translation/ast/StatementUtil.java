...
class SkeletonVisitor extends ASTVisitor {
	...
	public boolean preVisit2(ASTNode node) {
		...
		if (node instanceof Block) {
			r = new Range(null, node.getStartPosition(), node.getStartPosition() + node.getLength()-1, OperationKind.remove);
		}
		if (node instanceof SimpleName || node instanceof StringLiteral || node instanceof CharacterLiteral || node instanceof NumberLiteral || node instanceof TypeLiteral) {
			...
			if (ei == null) {
				ei = new ElementInfo(record.size(), cnt);
				record.put(cnt, ei);
			}
			r = new Range(ei, node.getStartPosition(), node.getStartPosition() + node.getLength()-1, OperationKind.replace);
		}
		if (r != null) {
			ranges.add(r);
		}
	...
	public void postVisit(ASTNode node) {
		if (root == node) {
			...
			for (Range r : ranges) {
				parts.add(cnt.substring(start, r.buf_start));
