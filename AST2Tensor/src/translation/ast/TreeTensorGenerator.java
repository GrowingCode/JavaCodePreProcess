...
public class TreeTensorGenerator extends TreeVisitor {
	...
	public boolean PreVisit(TreeNode node) {
		...
		if (children_nodes != null && children_nodes.size() > 0) {
			curr_tensor.StorePrePostOrderNodeInfo(type_content_id, 0, -1, im.GetGrammarID(GrammarRecorder.GetGrammar(node)), TokenKindUtil.GetTokenKind(node));
		}
		return true;
	}
	...
	public void PostVisit(TreeNode node) {
		...
		curr_tensor.StorePrePostOrderNodeInfo(type_content_id, has_children ? 2 : 1, post_order_index, im.GetGrammarID(GrammarRecorder.GetGrammar(node)), TokenKindUtil.GetTokenKind(node));
	}
